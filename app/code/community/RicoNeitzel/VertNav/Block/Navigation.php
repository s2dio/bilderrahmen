...
class RicoNeitzel_VertNav_Block_Navigation extends Mage_Catalog_Block_Navigation
{
    ...
    protected function _getCategoryCollection()
    {
        ...
        $filterCatIds = Mage::getStoreConfig('catalog/vertnav/filtercat');
       
        if(trim($filterCatIds)){
             $filterArray=array();
            $ids = explode(',',$filterCatIds);
            if(is_array($ids)){
                foreach($ids as $id){
                    $filterArray[]['neq']=$id;
                }
            }else{
                $filterArray = array('neq'=>$ids);
            }
            $collection->addAttributeToFilter('entity_id',$filterArray);
        }
